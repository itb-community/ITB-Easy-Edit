...
local WEAPON_ICON_DEF = easyEdit.weapons:getIconDef()
...
local function onRecieve_image(reciever, sender)
	local unitImage = easyEdit.unitImage:get(sender.data.Image)
...
local function onRecieve_weapon(reciever, sender, weaponSlot)
	local weapon = easyEdit.weapons:get(sender.data.SkillList[weaponSlot])
...
local function buildFrameContent(parentUi)
	...
	uiEditBox = {
		id = createUiEditBox(),
		name = createUiEditBox(UiInputField),
	...
	local images_filtered = filter_table(easyEdit.unitImage._children, function(k, unitImage)
	...
	local weapons_filtered = filter_table(easyEdit.weapons._children, function(k, weapon)
	...
	local createNewUnit = UiInputField()
		...
		:beginUi()
			...
			:beginUi(UiBoxLayout)
				...
				:beginUi(UiWeightLayout)
					:width(1):heightpx(icon_weapon_height)
					:hgap(5)
	...
	function createNewUnit:onEnter()
		...
		if name:len() > 0 and easyEdit.units:get(name) == nil then
			if _G[name] == nil then
				_G[name] = easyEdit.units._baseMech:new()
	...
	createNewUnit.onFocusChangedEvent:subscribe(function(uiInputField, focused, focused_prev)
		if focused then
			uiInputField.textfield = ""
			uiInputField:setCaret(0)
			uiInputField.selection = nil
		else
			uiInputField.textfield = TITLE_CREATE_NEW_UNIT
		end
	end)
...
local function buildFrameButtons(buttonLayout)
	...
	function button:relayout()
		...
		for _, unit in ipairs(unitList.children) do
			if unit.data.edited then
				self.disabled = false
				break
			end
		end
