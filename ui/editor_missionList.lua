...
local decorate_button_mission = decorate.button.mission
...
local function buildFrameContent(parentUi)
	...
	local mission_iconDef = modApi.missions:getIconDef()
	...
	local content = UiWeightLayout()
		...
		:beginUi(Ui)
			...
			:beginUi(UiWeightLayout)
				...
				:add(createUiTitle("Mission Lists"))
		...
		:endUi()
		:beginUi(Ui)
			:widthpx(mission_iconDef.width * mission_iconDef.scale + 4 * PADDING + SCROLLBAR_WIDTH)
			:padding(PADDING)
			:beginUi(UiWeightLayout)
				:width(1)
				:vgap(8)
				:orientation(ORIENTATION_VERTICAL)
				:add(createUiTitle("Missions"))
				:beginUi(UiScrollArea)
					:decorate{ DecoFrame() }
					:beginUi(missions)
						:padding(PADDING)
	...
	for _, mission in pairs(missions_filtered) do
		local entry = UiDragSource(dragObject)

		entry
			:widthpx(mission_iconDef.width * mission_iconDef.scale)
			:heightpx(mission_iconDef.height * mission_iconDef.scale)
			:setVar("data", mission)
			:setCustomTooltip(tooltip.mission)
			:addTo(missions)

		decorate_button_mission(entry, mission)
	end
